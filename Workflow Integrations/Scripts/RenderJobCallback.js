...
function registerCallback(callbackName, callbackFunction) {
    ...
    if (!isCallbackRegistered) {
        alert(`Error: Failed to register '${callbackName}' callback!`);
    }
...
function deregisterCallback(callbackName) {
    ...
    if (!isCallbackDeregistered) {
        alert(`Error: Failed to deregister '${callbackName}' callback!`);
    }
...
function main(pluginId) {
    ...
    if (!project) {
        alert("Error: No current project exist, create a project and try again!");
        return;
    }
    ...
    if (!renderJobList || (renderJobList.length == 0)) {
        alert("Error: No render job exists in current project, add one render job and try again!");
        return;
    }
